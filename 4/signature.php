...
function ma_hoa_hexa(s){
  s_length = s.length;
  var s_hexa = "";
  // alert(String.fromCharCode(parseInt(s_hexa.charCodeAt(0).toString(16),16)));
  // parseInt(s_hexa.charCodeAt(0).toString(16),16)
  // alert(s_hexa.charCodeAt(0).toString(16));
  for(var i = 0; i < s.length; i++){
    var x = s.charCodeAt(i).toString(16);
    if(x.length == 1){
      x = "0" + x;
    }
    s_hexa += x;
...
function encryption(){
  ...
  alert("c: " + c);
  document.getElementById("inputTextToSave").value = ma_hoa_hexa(document.getElementById("inputTextToSave").value) + "\n" + ma_hoa_hexa(arr[0]) + "\n" + ma_hoa_hexa(arr[2]) + "\n" + ma_hoa_hexa(c.toString()) + "\n" + ma_hoa_hexa(date1);
  return c;
}
</script>
