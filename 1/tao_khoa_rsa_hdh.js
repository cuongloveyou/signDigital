...
function createKey(){
	createPrime();
	var p = prime[Math.floor(Math.random() * 10000)];
	var q = prime[Math.floor(Math.random() * 10000)];
	var tt = false;
	var x = 1;
	var d;
	//document.writeln(tt);
	//document.writeln(p+"  ,  "+q);
	var e;
	var n = p*q;
	phi_n = (p-1)*(q-1);

	while(!tt){
		e = Math.floor(Math.random() * 100000);
		tt = gcd(e, phi_n);
	}
	...
	// form.moduleNumber.value = n;
	// form.publicKey.value = e;
	// form.privateKey.value = d;
	/*
	form.t_p.value = p;
	form.t_q.value = q;
	form.t_n.value = n;
...
function submit1(){
	alert('moduleNumber = ' + document.getElementById("n").value);
    alert('plK = ' + document.getElementById("e").value);
    alert('pvK = ' + document.getElementById("d").value);
}
